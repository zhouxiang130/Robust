...
//      setTitleLeftImg();
		...
		switch (view.getId()) {
			...
			case R.id.mine_account_withdraw_tvSend:
				...
				if (balance != null) {
					...
					if (want < 20) {
						ToastUtils.showToast(getApplicationContext(), "提现金额不能小于20,请努力赚钱吧!");
						return;
					}
					if (isSend) {
						return;
	...
	private void doAsyncGetData() {
		...
		map.put("userId", mUtils.getUid());
		LogUtils.i("传输的值" + URLBuilder.format(map));
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/user/backmoney.act").tag(this)
				.addParams(Key.data, URLBuilder.format(map))
...
//                        mThread=null
	...
	private void doAsyncPost() {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/user/addBackMoney.act")
				.addParams(Key.data, URLBuilder.format(map))
