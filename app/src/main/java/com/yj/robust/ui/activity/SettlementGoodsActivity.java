...
import com.yj.robust.widget.Dialog.SettlementGoodsTicketDialog;
	...
	private void onMoneyChange() {
		...
		if (cbSwitch.isOpened()) {
			...
			if (total < rest) {
				...
				tvRest.setText("-" + rest + "元");
				tvRestAll.setText("可用余额抵扣0元");
			...
			if (!isTickAfterPics) {
				if (data != null) {
					total = Float.parseFloat(data.getOrderTotalMoney());
					tvRest.setText("");
					tvRestAll.setText("可用余额抵扣" + rest + "元");
...
//        map.put("addressId","");
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/payImmediately.shtm")
...
//		TextView tvStoreName;
	...
	private void payWithWechat() {
		...
		LogUtils.i("传输的值" + URLBuilder.format(map));
		try {
			OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/payImmediatelyAddOrder.shtm")
					.addParams("data", URLBuilder.format(map))
