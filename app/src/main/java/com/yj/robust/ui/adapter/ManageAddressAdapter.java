...
import com.yj.robust.ui.activity.MineAddressNewActivity;
...
public class ManageAddressAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	...
	private List<AddressEntity.AddressData.AddressList> mList = new ArrayList<>();

	public ManageAddressAdapter(Context mContext, List<AddressEntity.AddressData.AddressList> mList, UserUtils mUtils,
	...
	public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
		if (holder instanceof AddressViewHolder) {
			...
			((AddressViewHolder) holder).tvAddress.setText("收货地址：" + mList.get(position).getAddressArea() + mList.get(position).getAddressAreaDetail());
			...
			if (mList.get(position).getType().equals("1")) {
				...
				((AddressViewHolder) holder).tvEdit.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View view) {
						...
						intent.putExtra("detial", mList.get(position).getAddressAreaDetail());
	...
	private void doAsyncDeleteAddress(final int position) {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/user/userAddressDelete.act").tag(this)
	...
	private void setDefaultAddress(final int position) {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/user/updateAddressDefault.act").tag(this)
				.addParams(Key.data, URLBuilder.format(map))
				.build().execute(new Utils.MyResultCallback<NormalEntity>() {
			@Override
			public void onError(Call call, Exception e) {
