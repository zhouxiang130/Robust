...
/*		mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
	...
	@Override
	public void showDialogs(int type, GoodsEntity.GoodsData proDetailCoupon) {
...
//		goodDetailPresenter.doAsyncGetDetial();
	...
	@OnClick({R.id.goods_detial_buy, R.id.goods_detial_rl_like, R.id.goods_detial_rlcart, R.id.goods_detial_add, R.id.goods_detial_rl_return,
			R.id.goods_detial_rl_share, R.id.goods_detial_custom_service, R.id.goods_detial_store})
	public void onClick(View view) {
		switch (view.getId()) {
	...
	private void setAsynrRemind() {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/rushToBuyReminding.act")
...
//		userInfo.setFace(URLBuilder.getUrl(mUtils.getAvatar()));//头像
	...
	private void doAsyncGetDetial() {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/productDetails.shtm")
				.addParams("data", URLBuilder.format(map))
...
//									btnBuy.setEnabled(false);
	...
	private void doAsyncGetJudge() {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/commentTwo.act")
				.addParams("data", URLBuilder.format(map))
	...
	private void doAsyncGetInfo() {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/choShopInfo.act")
				.addParams("data", URLBuilder.format(map))
	...
	public void doAsyncCollect() {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/saveDeleteCollection.act")
				.addParams("data", URLBuilder.format(map))
				.tag(this).build().execute(new Utils.MyResultCallback<NormalEntity>() {
			@Override
			public void onBefore(Request request) {
	...
	private void doGetStyleData(String pro) {
		...
		LogUtils.i("传输的值" + URLBuilder.format(map));
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/proSkuImg.act")
				.addParams("data", URLBuilder.format(map))
...
//                    ToastUtils.showToast(GoodsDetailActivity.this, "添加成功");
	...
	private void doAddCart(String pro, String number) {
		...
		if (data.getShopId() != null) {
			map.put("shopId", data.getShopId());
		}

		map.put("proNumber", number);
		LogUtils.i("传输的值" + URLBuilder.format(map));
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/addShopCar.act")
				.addParams("data", URLBuilder.format(map))
				.tag(this).build().execute(new Utils.MyResultCallback<NormalEntity>() {
			@Override
			public void onBefore(Request request) {
	...
	private void doAsyncGetCart() {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/cartNumber.act")
				.addParams("data", URLBuilder.format(map))
