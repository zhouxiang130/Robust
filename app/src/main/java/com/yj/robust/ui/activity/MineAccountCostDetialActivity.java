...
import com.yj.robust.model.CostDetailEntity;
...
public class MineAccountCostDetialActivity extends BaseActivity {
    ...
    private void doAsyncGetWithdraw() {
        ...
        OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/user/backMoneyDetails.act").tag(this)
    ...
    private void doAsyncGetConsume() {
        ...
        LogUtils.i("传输的值" + URLBuilder.format(map));
        OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/user/usersMoneyConsumeDetails.act").tag(this)
                .addParams(Key.data, URLBuilder.format(map))
                .build().execute(new Utils.MyResultCallback<CostDetailEntity>() {
