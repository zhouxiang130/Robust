package com.yj.robust.ui.fragment.HomeGoodsListFrag;
...
import com.yj.robust.ui.adapter.HomeGoodsListAdapter;
...
import static com.yj.robust.ui.adapter.HomeGoodsListAdapter.SPAN_COUNT_ONE;
import static com.yj.robust.ui.adapter.HomeGoodsListAdapter.SPAN_COUNT_TWO;
...
//		mRecyclerView.addItemDecoration(new DividerItemDecoration(this, DividerItemDecoration.VERTICAL_LIST));
		...
		mAdapter.setOnItemClickListener(new HomeGoodsListAdapter.SpendDetialClickListener() {
			...
			public void onItemClick(View view, int postion) {
				...
				intent.putExtra("productId", mList.get(postion - 1).getProId());
...
//					ivClassIcon.setBackgroundResource(R.mipmap.shangpinliebao_fenlei);
	...
	private void doRefreshData() {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/homePage/productList.act")
			...
			public void onResponse(GoodsListEntity response) {
				if (response != null && response.getCode().equals(response.HTTP_OK)) {
					if (response.getData() != null) {
						...
						if (response.getData().getProJsonArray().size() != 0) {
							mList.clear();
							mList.addAll(response.getData().getProJsonArray());
							mAdapter.notifyDataSetChanged();
							mProgressLayout.showContent();
	...
	private void notifyActivity(GoodsListEntity.GoodsListData data) {
		...
		if (data != null) {
			if (data.getShopId() != null && !data.getShopId().equals("")) {
				if (data.getShopName() != null && !data.getShopName().equals("")) {
					...
					intent1.putExtra("shopImg", data.getShopImg());
					intent1.putExtra("shopName", data.getShopName());
					intent1.putExtra("shopType", data.getShopType());
					intent1.putExtra("shopTypeName", data.getShopTypeName());
					intent1.putExtra("receipt", data.getReceipt());
					intent1.putExtra("detailNumMonth", data.getDetailNumMonth());
			...
			public void onResponse(GoodsListEntity response) {
				if (response != null && response.getCode().equals(response.HTTP_OK)) {
					if (response.getData() != null) {
						...
						} else if (response.getData().getProJsonArray().size() == 0) {
