package com.yj.robust.widget.Dialog;
...
import com.yj.robust.ui.adapter.SizeDialogAdapter;
...
public class CustomSizeDialog extends Dialog {
	...
	private GoodsEntity.GoodsData mData;
	...
	public CustomSizeDialog(Context context, CustomSizeDialogViewGroup.OnGroupItemClickListener listener,
	                        GoodsEntity.GoodsData mData, List<GoodsSaleEntity.GoodsSaleData> mSale) {
	...
	private void setCustomDialog() {
		...
		if (mData.getReceipt() != null && !mData.getReceipt().equals("")) {
			if (mData.getReceipt().equals("1")) {
		...
		tvPrice.setText(mData.getProductCurrent());
		tvStock.setText(mData.getProductStock());
		productMaxpurchase = mData.getProductMaxpurchase();
		if (productMaxpurchase != null && !productMaxpurchase.equals("")) {
			tvLimit.setText("限购" + mData.getProductMaxpurchase() + "件");
		} else {
			tvLimit.setText("购买数量不限");

		}
		Glide.with(context)
				.load(URLBuilder.getUrl( mData.getProductListimg()))
				.centerCrop()
		...
		viewAdd.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				if (count >= Integer.parseInt(mData.getProductStock())) {
					ToastUtils.showToast(context, "库存不够咯 ^.^");
					return;
				}
				String productMaxpurchase = mData.getProductMaxpurchase();
				if (productMaxpurchase != null && !productMaxpurchase.equals("")) {
					if (count >= Integer.parseInt(mData.getProductMaxpurchase())) {
						ToastUtils.showToast(context, "最多只能购买" + mData.getProductMaxpurchase() + "件哦 ^.^");
						return;
					}
				}
				count++;
		...
		checkView(mData.getProductStock());
